...
*/
function applySimpleRoute (router) {
  for (var controller in radar.controller) {
    var ctrs = controller.split(' ')
    if (ctrs.length > 1) {
      var method = ctrs[0]
      var route = '/' + ctrs[1].replace(/^\//, '')
      router[method](route, radar.controller[controller])
    }
  }
}
